...
/**
 * Response from Notion.so's HTTP API.
...
/**
 * Options of NotionAgent
 * @typedef {Object} AgentOptions
 * @property {string} token - Login token (`token_v2` field in cookie).
 * @property {string} timeZone - User's timezone, default: `Asia/Taipei`.
 * @property {string} locale - User's locale, default: `en`.
 * @property {boolean} suppressWarning - Whether to hide warnings, default: `false`.
 * @property {boolean} verbose - Whether to show status messages.
 */

/**
 * An agent to interact with Notion.so's HTTP API.
 * @typedef {Object} NotionAgent
 * @property {Function} loadPageChunk
 * @property {Function} getAssetsJson
 * @property {Function} getRecordValues
 * @property {Function} loadUserContent
 * @property {Function} queryCollection
 * @property {Function} submitTransaction
 */
...
function NotionAgent(options = {}) {
  ...
  if (!suppressWarning && token.length === 0) {
  ...
  async function makeRequestToNotion(apiURL, requestData) {
    ...
    if (verbose) log(`Request ${apiURL}, data ${requestData.slice(0, 40)} ...`)
   ...
   * @returns {Promise.<APIResponse>} JSON object from response.
   */
  this.loadPageChunk = (pageID, chunkNo = 0, cursor = { "stack": [] }) => {
